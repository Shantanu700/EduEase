...
def get_flash_card_chain():
    ...
    prompt = PromptTemplate(template = prompt_template, input_variables = ["context", "word"])
    chain = load_qa_chain(model, chain_type="stuff", prompt=prompt)

    return chain

def user_input_word(user_word):
    embeddings = GoogleGenerativeAIEmbeddings(model = "models/embedding-001")
    
    new_db = FAISS.load_local("faiss_index", embeddings, allow_dangerous_deserialization=True)
    docs = new_db.similarity_search(user_word)

    chain = get_flash_card_chain()

    
    response = chain(
        {"input_documents":docs, "word": user_word}
        , return_only_outputs=True)
